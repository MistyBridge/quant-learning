# -*- coding: utf-8 -*-
"""
烟蒂股策略 v5 · 等权满仓·加大分散 —— 聚宽平台

【说明】这是"小市值 + 高股息可持续"代理口径、**纯分散版本**:与 v1 原版完全相同的选股/调仓逻辑,
    仅把候选池和持仓数量加大(top_n 10->20、pool_size 1000->1500),**等权、满仓、不降仓、不加任何凯莉/风控层**。

【目的】用"分散"降波动/回撤,而不牺牲满仓的收益强度——这是此前 v2/v3/v4 验证过的最优方向(非凯莉)。
...
def initialize(context):
    ...
    # ---------- 选股（加大分散）----------
    g.pool_size = 1500        # 市值最小的 N 只（候选池，v1 为 1000）
    g.top_n = 20              # 持有数量（v1 为 10，加大分散）
...
def sell_holdings(context, target, prev):
    ...
    for stock, pos in list(context.portfolio.positions.items()):
        ...
        if cur.is_st or 'ST' in name.upper() or '*' in name or '退' in name:
            ...
            log.info('烟蒂股v5：退市/ST 卖出 %s' % stock)
            continue
        avg_cost = pos.avg_cost
        last = cur.last_price
        if avg_cost > 0 and last > 0 and (last / avg_cost - 1) <= g.stop_loss:
            order_target(stock, 0)
            log.info('烟蒂股v5：止损卖出 %s (%.1f%%)' % (stock, (last / avg_cost - 1) * 100))
            continue
        if stock not in target_codes:
            order_target(stock, 0)
            log.info('烟蒂股v5：调出 %s' % stock)
...
def buy_holdings(context, target):
    ...
    cd = get_current_data()
    weight = 1.0 / len(codes)   # 等权、满仓（分散到 top_n 只）
    for stock in codes:
        cur = cd[stock]
...
def record(context, target):
    try:
        ...
        log.info('烟蒂股v5：净值 %.2f | 现金 %.2f | 持仓 %d | 候选 %d'
