...
- 如果传入了fq_ref_date，则以fq_ref_date为基准日进行复权 ； 
...
用法注释：
1.指标>80 时，回档机率大；指标100 时，股价易反转下跌；J<0 时，股价易反转上涨；
...
- 量比是指股市开市后平均每分钟的成交量与过去N个交易日平均每分钟成交量之比
- 当天开盘截至到当前时刻（check_date），平均每分钟的成交手数，除以过去N天平均每分钟的成交手数，这里的unit就是分钟级别的，所以不用unit属性；
