...
The older `market-regime-daily` example predates this strict coverage contract
and retains its historical fixture layout. `trade-memory-loop` is now generated
by the executable replay harness.
...
## Executable workflow replay (Issue #294, Coverage 3/11)

Three of the eleven canonical workflows are generated and checked by the
...
The fluency replay runs the real `build_model_book.py` CLI for steps 1–3. The
...
postmortem APIs against temporary state. All three use bundled fictional inputs
...
freezes the other eight current workflows as Coverage 3/11 deferrals linked to Issue
